...
state = State()
state.show_config = not config_manager.is_configured()
...
.orb-idle {
  ...
  flex-shrink: 0;
    ...
    main_container = ui.column().classes("w-full max-w-7xl mx-auto p-4").bind_visibility_from(
        state, "show_config", lambda v: not v
    )
    with main_container:
        # ── Header ────────────────────────────────────────────────
        with ui.row().classes("w-full items-center justify-between"):
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                status_pill = ui.label("").classes("text-sm").style(
                    f"color: {MUTED}; min-width: 100px;"
                )

                def _show_settings():
                    state.show_config = True

                ui.button("Settings", on_click=_show_settings).props(
...
def main():
    ui.run(
        root=build_ui,
        title="CheckMate — RFP Compliance Checker",
        favicon="CheckMate",
        dark=False,
        reload=False,
        native=True,
